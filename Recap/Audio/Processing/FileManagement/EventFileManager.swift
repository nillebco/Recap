...
protocol EventFileManaging {
    ...
    func writeTranscription(_ transcription: String, for eventID: String, duration: TimeInterval?, model: String?, sources: [AudioSource]) throws
...
extension EventFileManager {
    ...
    /// Write enhanced transcription data to markdown file with metadata
    func writeTranscription(_ transcription: String, for eventID: String, duration: TimeInterval?, model: String?, sources: [AudioSource]) throws {
        let transcriptionURL = createTranscriptionFileURL(for: eventID)
        let markdownContent = formatEnhancedTranscriptionAsMarkdown(transcription, eventID: eventID, duration: duration, model: model, sources: sources)
        try markdownContent.write(to: transcriptionURL, atomically: true, encoding: .utf8)
        logger.info("Written enhanced transcription to: \(transcriptionURL.path)")
    }
    ...
    private func formatEnhancedTranscriptionAsMarkdown(_ transcription: String, eventID: String, duration: TimeInterval?, model: String?, sources: [AudioSource]) -> String {
        let formatter = DateFormatter()
        formatter.dateFormat = "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'"
        formatter.timeZone = TimeZone(abbreviation: "UTC")
        let timestamp = formatter.string(from: Date())

        // Clean and prettify the transcription text
        let prettifiedTranscription = TranscriptionTextCleaner.prettifyTranscriptionText(transcription)
